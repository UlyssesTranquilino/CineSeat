      ...
      <Slider {...settings}>
        {movies.slice(0, 5).map((movie) => (
          <Link to={`/movie/${movie._id}`} key={movie._id}>
            <div className="relative h-70 md:h-100 bg-black overflow-hidden rounded-md">
              <div
              ...
              >
                ...
                <div className="flex items-start">
                  <Link to={`/movie/ticket/${movie._id}`}>
                    <button className="bg-[#FFD700] rounded-sm w-30 md:w-35 lg:w-40 text-sm text-black cursor-pointer font-semibold p-1 md:text-md lg:text-lg hover:shadow-lg shadow-yellow-500/50 transition-all duration-300 ease-in-out">
                      Book Tickets
                    </button>
                  </Link>
                </div>
